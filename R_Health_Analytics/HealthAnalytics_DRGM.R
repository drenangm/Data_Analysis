...
set.seed(1)
index <- sample(1:nrow(dados_final), dim(dados_final)[1] * 0.7)
...
hist(residuos, xlab = 'Residuos', main = 'Sobreviventes de 1 a 3 anos')
...
set.seed(1)
index <- sample(1:nrow(dados2), dim(dados2)[1] * 0.7)
...
#outra forma de se obter os dados numericos
# dados_treino_num_exp <- dados_treino[,unlist(lapply(dados_treino, is.numeric))]
# dim(dados_treino_num_exp)
...
names(df_final) = c(variaveis_numericas_nomes,
                    ...
                    paste('LISTYR', c(1:18), sep = ''))

dim(df_final)
View(df_final)

# Divisão em dados de treino e teste
index2 <- sample(1:nrow(df_final), dim(df_final)[1] * 0.7)
